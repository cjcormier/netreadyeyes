...
class UI:
    def __init__(self, root, app):
        ...
        self.video_label = ttk.Label(self.main_frame, text="Video Feed", background="blue")
        ...
        self.webcam_label = ttk.Label(self.video_label, text="Select Webcam:")
        self.webcam_combobox = ttk.Combobox(self.control_frame, values=self.app.available_webcams)
        ...
        # self.display_mode_combobox = ttk.Combobox(self.control_frame, values=["none", "thresholding", "unfiltered contours", "filtered contours", "rectangular_contours"])
        self.start_button = ttk.Button(self.control_frame, text="Start Webcam", command=self.start_webcam, bootstyle=SUCCESS)
        self.stop_button = ttk.Button(self.control_frame, text="Stop Webcam", command=self.stop_webcam, state=DISABLED, bootstyle=DANGER)
        ...
        # self.proc_period_label = ttk.Label(self.control_frame, text="Image processing_period (ms):")
        # self.proc_period_slider = ttk.Scale(self.control_frame, from_=10, to_=2000, orient=HORIZONTAL, command=self.app.update_proc_period)
        ...
        self.webcam_label.grid(row=0, column=0, padx=self.padx, pady=self.pady)
        self.webcam_combobox.grid(row=0, column=1, padx=self.padx, pady=self.pady)
        # self.detect_mode_label.grid(row=3, column=0, padx=self.padx, pady=self.pady)
        # self.detect_mode_combobox.grid(row=3, column=1,padx=self.padx, pady=self.pady)
        ...
        self.start_button.grid(row=2, column=0, padx=self.padx, pady=self.pady)
        self.stop_button.grid(row=2, column=1, padx=self.padx, pady=self.pady)
        # self.display_mode_label.grid(row=3, column=0, padx=self.padx, pady=self.pady)
        # self.display_mode_combobox.grid(row=3, column=1,padx=self.padx, pady=self.pady)

        self.img_on_process_frame = None  # Keep track of the current image on the canvas

        # Default to first webcam in the list
        if len(self.app.available_webcams) >= 2:
            self.webcam_combobox.set(self.app.available_webcams[2]) #default to #2 - camlink 4k on eric's systems
        elif self.app.available_webcams:
            self.webcam_combobox.set(self.app.available_webcams[0])
        ...
        # self.proc_period_label.grid()


        # self.proc_period_slider.set(500)  # Default in milliseconds
        # self.proc_period_slider.grid()
    ...
    def start_webcam(self):
        ...
        self.video_source = cv2.VideoCapture(webcam_index, cv2.CAP_DSHOW)  # Use DirectShow backend
        ...
        # Log message
        self.log_msg(f"Started webcam: {selected_webcam} (ROI centered at: {self.roi_x}, {self.roi_y})")
